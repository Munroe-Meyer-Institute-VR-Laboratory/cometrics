'''
https://github.com/WelkinU/ThreadedVideoLoader/blob/master/VideoLoader.py
...
class VideoLoader():
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, slice):
            ...
            if self.frame_count > idx >= -self.frame_count and isinstance(idx, int):
                ...
                self.cap.set(self.pos_frames_number,
                             cur_frame_pos)  # reset current frame position so this method doesn't interfere with __iter__() or __next__()
    ...
    def __iter__(self):
        ...
        if self.use_threading:
            ...
            while ret:
                ...
                yield self.apply_transform(frame)
        self.cap.set(self.pos_frames_number,
                     0)  # reset frame position to 0, in case __iter__() is called multiple times sequentially
    ...
    def apply_transform_to_video(self, output_video_path=None, output_video_codec=None, fps=None, start=0, end=None,
                                 step=1, enable_start_stop_with_keypress=False):
    ...
    def dump_frames_from_video(self, output_folder, file_format='frame{:05d}.jpg', start=0, end=None, step=1,
                               enable_start_stop_with_keypress=False):
        ...
        for idx, frame in zip(range(start, end, step), self.get_series_of_frames_iterator(start, end, step)):
            cv2.imwrite(output_folder + '/' + file_format.format(idx),
                        frame)  # can put this in a thread for speed when using webcam
        ...
        while ret and self.thread_started:
            ...
            try:
                self.frame_queue.put(frame, block=True,
                                     timeout=1)  # timeout is in seconds, on last read, None is put into the Queue conveniently
            except queue.Full:
                ...
                if not self.thread_started:
                    ...
                    if not self.first_queue_full_warning_displayed:
                        ...
                        print(
                            'Warning background thread has filled up frame queue storage. Future frames may be dropped if input is a video stream.')
